#!/usr/bin/env python
"""
Debug Column Lineage Extraction

This script tests column lineage extraction for different SQL dialects.
"""
...
from tools.sql_tools.dialects.tsql.tsql_dialect import TSQLDialect
from tools.sql_tools.dialects.postgres.postgres_dialect import PostgreSQLDialect
from tools.sql_tools.dialects.dbt.dbt_dialect import DBTDialect
...
def test_dialect_column_lineage(dialect, sql_file_path):
    """Test column lineage extraction for a dialect"""
    logger.info(f"Testing {dialect.__class__.__name__} column lineage extraction")
    ...
    if 'file_path' in sig.parameters:
        ast, errors = dialect.parse_sql(sql_content, file_path=sql_file_path)
    else:
        ast, errors = dialect.parse_sql(sql_content)
    
    if errors:
        logger.error(f"Errors parsing SQL: {errors}")
        return None
    
    # Use SQLGlotLineageExtractor directly
    lineage_extractor = SQLGlotLineageExtractor()
    
    # Extract column lineage
    column_lineage = lineage_extractor.extract_column_lineage(ast, file_path=sql_file_path)
...
def main():
    """Main function"""
    logger.info("Starting column lineage debug script")
    
    # Create test files
    test_files = write_test_files()
    
    # Initialize dialects
    tsql_dialect = TSQLDialect()
    postgres_dialect = PostgreSQLDialect()
    dbt_dialect = DBTDialect()
    
    # Test TSQL dialect
    logger.info("=== Testing T-SQL Column Lineage ===")
    tsql_results = test_dialect_column_lineage(tsql_dialect, test_files["tsql"])
    
    # Test Postgres dialect
    logger.info("\n=== Testing PostgreSQL Column Lineage ===")
    postgres_results = test_dialect_column_lineage(postgres_dialect, test_files["postgres"])
    
    # Test DBT dialect
    logger.info("\n=== Testing DBT Column Lineage ===")
    dbt_results = test_dialect_column_lineage(dbt_dialect, test_files["dbt"])
    
    # Log summary
    logger.info("\n=== Column Lineage Testing Summary ===")
    logger.info(f"T-SQL: {len(tsql_results.get('target_columns', []))} target columns, {len(tsql_results.get('column_relationships', []))} relationships")
    logger.info(f"PostgreSQL: {len(postgres_results.get('target_columns', []))} target columns, {len(postgres_results.get('column_relationships', []))} relationships")
    logger.info(f"DBT: {len(dbt_results.get('target_columns', []))} target columns, {len(dbt_results.get('column_relationships', []))} relationships")
